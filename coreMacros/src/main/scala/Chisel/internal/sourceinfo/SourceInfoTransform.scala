...
class WhenTransform(val c: Context) extends SourceInfoTransformMacro {
  import c.universe._
  def apply(cond: c.Tree)(block: c.Tree): c.Tree = {
    q"$thisObj.do_apply($cond, $block)($implicitSourceInfo)"
  }
...
/** "Automatic" source information transform / insertion macros, which generate the function name
  ...
  */
class SourceInfoTransform(val c: Context) extends SourceInfoTransformMacro {
  import c.universe._
  ...
  /** Transforms a call of the form obj.fn(that) into obj.do_fn(sourceInfo, that). Single argument
    * must be named that.
    */
  def noArg(): c.Tree = {
